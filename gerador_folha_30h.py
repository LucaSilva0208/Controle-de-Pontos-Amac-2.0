...
class GeradorFolha30h(GeradorBase):
    def gerar(self, funcionario, mes, ano, caminho_saida_pdf, recessos_globais=None):
        ...
        doc = Document()
        # Margens (Pode ajustar aqui sem afetar a 40h)
        ...
        # --- 3. DADOS DO FUNCIONÁRIO (LAYOUT 30H) ---
        ...
        self.celula_texto(table_info.cell(1, 1), "Carga Horária: 30h (Intervalo: 15min)", align=WD_ALIGN_PARAGRAPH.LEFT, size=9)
        ...
        # --- 4. TABELA DE PONTO 30H ---
        # === MODELO 30H (10 Colunas) ===
        table = doc.add_table(rows=0, cols=10)
        table.style = 'Table Grid'
        table.autofit = False
        table.allow_autofit = False
        
        # AQUI ESTÃO AS MEDIDAS (Total ~15.4cm)
        # Dia(1.5) Ent(1.5) Rub(0.8) Sai(1.5) Rub(0.8) Int(1.5) Rub(0.8) HE(1.5) HE(1.5) Ass(4.0)
        larguras = [1.9, 1.9, 1.9, 1.9, 1.9, 1.9, 1.9, 1.9, 1.9, 1.9]
        
        # Header 1
        row_h1 = table.add_row()
        self.definir_altura_linha(row_h1, 0.7)
        self.celula_texto(row_h1.cells[0], "Dados", size=7)
        self.celula_texto(row_h1.cells[1], "Jornada Diária", size=7)
        row_h1.cells[1].merge(row_h1.cells[4]) # Ent, Rub, Sai, Rub
        self.celula_texto(row_h1.cells[5], "Intervalo (15min)", size=7)
        row_h1.cells[5].merge(row_h1.cells[6]) # Int, Rub
        self.celula_texto(row_h1.cells[7], "H. Extra", size=7)
        row_h1.cells[7].merge(row_h1.cells[8]) # HE Ent, HE Sai
        self.celula_texto(row_h1.cells[9], "Visto/Obs", size=7)

        # Header 2
        row_h2 = table.add_row()
        self.definir_altura_linha(row_h2, 0.5)
        titulos = ["Dia/Sem", "Entr", "Rub", "Saída", "Rub", "Horário", "Rub", "Entr", "Saída", "Assinatura"]
